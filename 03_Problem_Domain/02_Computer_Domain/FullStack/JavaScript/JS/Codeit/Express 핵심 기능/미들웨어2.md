...
```js
//참고로 유저라우터로 똑같은 이름으로 헸다면 코드상 위치순서

adminRouter.use((req,res,next) => {
  if(req.get('Authorization') === 's3cret') {
    next();
  } else {
...
```
> req.get(HEADER_FIELD) 메소드를 사용해서 특정해더의 값을 비교할수있으며 이로
> 해당 권한을 가진 사용자를 통과할지 못하게할지 코드를 짤수있다.

## Express 프로젝트 구조와 모듈화
여전히 하나의 파일로 작성하니 불편하다, express는 가볍고 유연해서 정답이없다.
```js
// routes/product.js
// 만들었던 product코드를 옮긴다.
...
```js
// middlewars/always.js
// 앱 객체에 아규먼트로 전달되고 있는 미들웨어함수를 옮긴다.
// 그리고 다른 파일에서 사용할수있게 export
export default function (req,res,next) => {
	console.log('나는 항상 싫행!');
	next();
}
...
import always from './middlewars/always.js';

app.use(alway); // 미들웨어를 전달해준다.

```
> 이제 앱,라우터,미들웨어로 3가지로 분리되었다(모듈화)
팀의 가치관 한경 모두 다르기에 어떤 구조가 효율적인지는 정답이 없을수있다.